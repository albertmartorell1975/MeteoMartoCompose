...
1. Gateway Diagnosis (STRICT): Every new feature or significant change MUST start with the Diagnosis & Clarification phase defined in [.agents/rules.md](.agents/rules.md). If any part of the request is ambiguous or lacks technical detail, the agent MUST stop and ask the user instead of making assumptions.
...
1.  **Notification Protocol (MANDATORY)**: Before making ANY change to the **governance files** (any `.md` file inside the `.agents/` directory), the agent **MUST notify the user**, explain the intended action, and wait for explicit approval. Changes to the codebase (Kotlin, XML, configurations) can be performed autonomously, but the agent **MUST NOT commit any changes unless explicitly commanded by the user in the chat**.
...
4.  **Documentation Sync (MANDATORY)**: Any addition, removal, or update of a Skill must be immediately reflected in the `.agents/skills/README.md` file. The agent is responsible for keeping this index organized into the 3 defined groups (DNA, Android CLI, External Experts) to ensure expert knowledge traceability.
